...
func _ready():
	...
	Ally = get_parent()
	AllyParent = Ally.get_parent()
	MagicShotSound = Ally.get_node("MagicShotSound")
	
	for i in pool_size:
		instantiate_bullet()
	
func _physics_process(delta: float) -> void:
	var direction = Vector2.RIGHT.rotated(Ally.rotation)
...
func instantiate_bullet():
	...
	current_bullet.speed = 500.0
	_ready_bullets_pool.append(current_bullet)
	AllyParent.call_deferred("add_child", current_bullet)
